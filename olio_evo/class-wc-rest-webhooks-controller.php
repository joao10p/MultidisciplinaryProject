...
class WC_REST_Webhooks_Controller extends WC_REST_Webhooks_V2_Controller {
	...
	function woocomm_add_to_cart($param) {
		...
		if (null === WC()->cart) {
			...
			// We need to force a refresh of the cart contents from session here (cart contents are 
			// normally refreshed on wp_loaded, which has already happened at this point)
			WC()->cart->get_cart();
		}
